...
public class GenerateRuleClass {
	static void exetute(String factName, List<String> sigs, List<String> fields, List<String> vars,
		...
		// initial ListGameObjectName
			...
			Constructor.addAnnotation(ArrayListType);
			Constructor.addStatement("ListGameObjectName = new ArrayList<String>()");
			ConstructorPara.addAnnotation(ArrayListType);
			ConstructorPara.addStatement("ListGameObjectName = new ArrayList<String>()");
		// initial listGameObjectName
		for (String s : sigs) {
		...
		MethodSpec.Builder execute = MethodSpec.methodBuilder("execute").addModifiers(Modifier.PUBLIC)
				.returns(Formula.class).addAnnotation(variableKodKod);
		// variable in fact
		for (String v : vars) {
